...
namespace Mango.Services.ShopingCartApi.Controllers
{
	...
	public class CartApiController : ControllerBase
	{
		...
		private readonly IRabbitMqCartMessageSender messageBus;
		...
		public async Task<ResponseDto> GetCart(string userId)
		{
			try
			{
				...
				foreach (var item in cartDto.CartDetails)
				{
					...
					try
					{
						var productX = productDtos.First(u => u.ProductId == item.ProductId);
						item.Product = productX;
						cartDto.CartHeader.CartTotal += (item.Count * item.Product.Price);
					}
					catch (Exception ex)
					{
						await CartRemove(item.CartDetailsId);
					}
				}
		...
		public async Task<ResponseDto> EmailCartRequest([FromBody] CartDto cartDto)
		{
			try
			{
				messageBus.SendMessage(cartDto, configuration.GetValue<string>("TopicAndQueueNames:EmailShoppingCartQueue")); // name of queue from Azure
				_responseDto.Result = true;
			}
			catch (Exception ex)
			{
